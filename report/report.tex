...
This work investigates the Ethereum network and more particulary focuses on the transactions of Ether. Various graph analysis tools are used to obtain insights about this network. First of all, a descriptive graph analysis of the network is performed, followed by an analysis of the major players in network (called \textit {whales}), to address their role and influence within the network. Finally, we investigate the community structure of the network. Six different snapshots of the network are used, starting in January 2017 and ending in March 2021. This allows us to identify evolutions over time.
...
\item{\textbf{Scalability:} due to the size of our networks, scalability has already been an important issue during our analysis. First of all, our data collection will not scale well, because the Etherscan API severly limits the number of requests per second. It is however possible to sync a personal node to the netwerk, and query locally. Because we used a fairly large network, most metrics and methods used have excellent time complexities. We summarize these in the table below. Assume $V$ is the number of nodes, $E$ is the number of edges, and $d$ is the average node degree.
