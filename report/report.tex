...
In this part the snapshots are all visualized to see whether the characteristics mentioned in 5.2 and shown on Figure 5 can also be obtained visually. 
For the visualization of the network Gephi is used because of its flexibility and representational power. 
On the other hand, for the visualizations, Gephi's limitations have also come to the forefront, the size of the snapshots was too large to be workable, that is why the network was limited for the visualizations. 

Figure 9 is a representation of the network of January 2017 with 39 281 nodes and 50 154 edges. Figure 10 is a representation of July 2021 with 49 125 nodes and 50 034 edges. The used layout algorithm is OpenOrd because of its ability to distinguish clusters in an orderly and convenient way and is able to work well with a lot of nodes. In the second step the Yifan Hu layout is used on the OpenOrd graph representation. This is a very fast algorithm with a good quality on large graphs. It combines a force-directed model with a graph coarsening technique to reduce the complexity. The combination of these two gave actually a very clear representation of the network \footnote{https://gephi.org/users/tutorial-layouts/}.

The colors of the graph can be seen as the main clusters which are obtained from the Leiden algorithm where the modularity is used as the final metric to disitinguish the clusters. Around 70\% of the nodes are included in a colored cluster. 



\begin{Figure}
\centering
\includegraphics[scale=0.45]{figures/Jan_17_edges.png}
\label{clusters}
...
\caption{Caption}
\label{fig:my_label}
\includegraphics[scale=0.45]{figures/Marc21_edges.png}
\label{clusters}
...
First of all validity is discussed. In terms of external and construct validity we had to make some choices to be able to work with a manageable data set. We had to limit the graph and this could have implications when the network is reconstructed without any restrictions on the number of nodes and edges. Although we have selected a significant network, based on a BFS, this can still create a bias. The size of the selected snapshots should compensate for this to some extent. Another point to keep in mind is the reliability of the graph analysis. We tried to handle this issue by using several metrics for the clustering and the community detection. The third measure to address is the scalability. All code was run on Google Collaborate with the help of the Igraph library, although some metrics took a considerable time to be calculated this could still be done in considerable time (MSS NOG AANVULLEN???). 

\section{Conclusion}

In this work, the money flow of Ethereum was examined using a graph analysis.  One of the first findings is that  transaction  volumes  have  not changed  much  over  time, only in 2017 these transactions were a bit higher. A second point of interest in the analysis was the small-world characteristics. We did obtain a very low clustering coefficient within the network, but on the other hand the average geodesic path length did show signs of a small-world network. 
